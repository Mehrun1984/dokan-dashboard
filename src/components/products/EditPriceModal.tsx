...
export default function EditPriceModal({ product, isOpen, isVip, onClose, onSuccess }: EditPriceModalProps) {
  ...
  const onSubmit = async (data: PriceFormData) => {
    try {
      const payload = isVip ? data : { regular_price: data.regular_price };
      const updated = await dokanService.updateProductPrices(product.id, payload);
      onSuccess(updated);
      onClose();
    } catch (error) {
